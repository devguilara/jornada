    ...
    <div class="py-12 bg-gray-900 text-gray-200 font-sans min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-800 rounded-3xl shadow-xl p-8">
                <div class="mb-8">
                    <h3 class="text-4xl font-extrabold tracking-tight text-white font-poppins">Criar Nova Playlist</h3>
                    <p class="mt-2 text-sm text-gray-400">Preencha os detalhes para a sua nova playlist musical.</p>
                </div>

                <form method="POST" action="{{ route('playlists.store') }}" class="mt-6 p-6 bg-gray-800 rounded-2xl shadow-inner-xl-dark">
                    @csrf

                    <div>
                        <x-input-label for="name" :value="__('Nome da Playlist')" class="text-gray-400" />
                        <input id="name" class="block mt-1 w-full rounded-md border-transparent bg-gray-700 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 focus:ring-offset-gray-800" type="text" name="name" :value="old('name')" required autofocus />
